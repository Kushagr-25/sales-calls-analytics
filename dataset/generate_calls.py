...
import uuid, json, random, os, time, datetime as dt
...
PRODUCTS = ["SaaS CRM subscription", "enterprise cloud backup", "AI-powered chatbot plan",
            "premium credit card", "solar panel installation", "health-tech wearable"]
...
def single_call(gen, local):
    ...
    prompt = craft_prompt(agent_id, customer_id,
...
def main(n_calls, out_dir, model, local):
    ...
    with open(out_dir / "calls.jsonl", "w", encoding="utf-8") as fout:
        for _ in tqdm(range(n_calls), desc="Generating calls"):
            ...
            # save raw transcript
            with open(raw_dir / f"{call['call_id']}.txt", "w", encoding="utf-8") as fraw:
